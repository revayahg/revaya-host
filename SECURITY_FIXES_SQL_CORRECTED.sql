-- SECURITY FIXES FOR PRODUCTION DEPLOYMENT (CORRECTED VERSION)
    ...
    LOOP
        BEGIN
            INSERT INTO public.event_dates (
                ...
                event_id_param,
                (date_item->>'date')::DATE,
                date_item->>'startTime',  -- Keep as TEXT, don't cast to TIME
                date_item->>'endTime',    -- Keep as TEXT, don't cast to TIME
                NOW(),
                auth.uid()
            );
...
RETURNS TABLE(
    id UUID,
    event_date DATE,
    start_time TEXT,  -- Changed from TIME to TEXT to match schema
    end_time TEXT     -- Changed from TIME to TEXT to match schema
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    -- Note: access_codes table only has 'active' column, not 'is_active'
    RETURN EXISTS (
        SELECT 1 
        FROM public.access_codes 
        WHERE code = TRIM(code_param) 
        AND active = TRUE
        -- No expires_at column in the schema
    );
EXCEPTION
    WHEN OTHERS THEN
        RETURN FALSE;
