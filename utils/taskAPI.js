...
window.TaskAPI = {
    ...
    async createTask(taskData) {
        ...
        try {
            ...
            const validColumns = ['event_id', 'title', 'description', 'category', 'status', 'priority', 'due_date', 'assigned_to'];
            const newTask = {};
                ...
                if (taskData.hasOwnProperty(column)) {
                    ...
                    switch (column) {
                        ...
                        case 'category':
                            ...
                            break;
                        case 'status':
                            newTask[column] = taskData[column] || 'pending';
                            break;
                        case 'priority':
            ...
            if (taskData.assigned_to_type === 'user_id' && taskData.assigned_to) {
                try {
                    ...
                    if (notifError) {
                        ...
                        if (window.EventBus && window.EventBus.emit) {
                            window.EventBus.emit('notificationCreated', { notification });
                        }
                ...
                try {
                    ...
                    const assignmentToken = window.notificationAPI?.generateTaskAssignmentToken() || `task_${Date.now()}_${Math.random().toString(36).substring(2)}`;
    ...
    async updateTask(taskId, taskData) {
        try {
            ...
            // Only include valid task columns to prevent schema errors
            const validColumns = ['title', 'description', 'category', 'status', 'priority', 'due_date', 'assigned_to'];
            const updates = {
            ...
            if (data.assigned_to && data.assigned_to_type === 'user_id') {
                try {
                    ...
                    if (notificationError) {
                        ...
                        if (window.EventBus && window.EventBus.emit) {
                            window.EventBus.emit('notificationCreated', { notification: notificationPayload });
                        }
                        window.dispatchEvent(new CustomEvent('notificationCreated', { 
                            detail: { notification: notificationPayload } 
                        }));
                    }
                } catch (notificationError) {
                }
                    ...
                    if (window.unifiedNotificationService) {
                        ...
                        if (userEmail) {
                            await window.unifiedNotificationService.sendNotification({
                                email: userEmail,
                                notification_type: notificationPayload.type,
                                event_id: notificationPayload.event_id,
                                event_name: data.title || 'Event', // Use event title if available
                                task_title: notificationPayload.metadata?.task_title,
                                task_status: notificationPayload.metadata?.new_status,
                                message: notificationPayload.message,
                                title: notificationPayload.title,
                                accept_url: `https://revayahost.com/#/event/view/${notificationPayload.event_id}`
                            });
