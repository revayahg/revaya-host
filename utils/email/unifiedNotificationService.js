...
const unifiedNotificationService = {
  ...
  resolveLink(type, eventId, metadata = {}) {
    try {
      if (window.notificationAPI?.getNotificationLink) {
        return window.notificationAPI.getNotificationLink({ type, eventId, metadata });
      }
      if (window.NotificationLinkUtils?.getNotificationLink) {
        return window.NotificationLinkUtils.getNotificationLink({ type, eventId, metadata });
      }
    } catch (error) {
      console.warn('unifiedNotificationService: link resolution failed, falling back.', error);
    }
    return null;
  },
  ...
  async sendTaskAssignmentEmail(assigneeEmail, assignerName, eventName, taskData) {
    try {
      ...
      const linkInfo = this.resolveLink('task_assigned', notificationData.event_id, {
        task_assignment_token: notificationData.task_assignment_token,
        task_id: taskData.id,
        task_title: taskData.title
      });
      if (linkInfo?.url) {
        notificationData.accept_url = linkInfo.url;
      }
      if (linkInfo) {
        notificationData.link = linkInfo;
      }
      
      // Debug: Log the notification data being sent
      console.log('🔍 Unified notification service debug - start_date:', taskData.start_date);
      console.log('🔍 Unified notification service debug - due_date:', taskData.due_date);
      console.log('🔍 Unified notification service debug - full data:', JSON.stringify(notificationData, null, 2));
  ...
  async sendCollaboratorInvitationEmail(email, inviterName, eventId, eventName, invitationToken, permissionLevel = 'viewer') {
    try {
      ...
      const linkInfo = this.resolveLink('collaborator_invitation', notificationData.event_id, {
        invitation_token: invitationToken,
        permission_level: permissionLevel
      });
      if (linkInfo?.url) {
  ...
  async sendChatMessageEmail(email, senderName, eventName, eventId, messagePreview, userId) {
    try {
      ...
      const linkInfo = this.resolveLink('chat_message', eventId, {
        sender_name: senderName
      });
      if (linkInfo?.url) {
        notificationData.accept_url = linkInfo.url;
  ...
  async sendEventUpdateEmail(email, eventName, eventId, updateType, updatedBy, updateDescription) {
    try {
      ...
      const linkInfo = this.resolveLink('event_updated', eventId, {
        tab: updateType === 'budget_changed' ? 'budget' : undefined
      });
      if (linkInfo?.url) {
        notificationData.accept_url = linkInfo.url;
      }
  ...
  async sendCollaboratorStatusChangeEmail(email, eventName, eventId, statusChange, updatedBy, statusDescription) {
    try {
      ...
      const linkInfo = this.resolveLink('collaborator_status_changed', eventId, {
        status_change: statusChange
      });
      if (linkInfo?.url) {
        notificationData.accept_url = linkInfo.url;
  ...
  async sendVendorInvitationEmail(email, vendorName, eventId, eventName, acceptUrl, declineUrl) {
    try {
      ...
      const linkInfo = this.resolveLink('vendor_invitation', eventId, {
        vendor_name: vendorName,
        accept_url: acceptUrl,
        decline_url: declineUrl
      });
      if (linkInfo?.url && !notificationData.accept_url) {
        notificationData.accept_url = linkInfo.url;
      }
      if (linkInfo) {
        notificationData.link = linkInfo;
