...
- [ ] Deploy updated `send-onboarding-email` Edge Function with unsubscribe token generation and environment variable URL
...
#### 3.4 Security Enhancements - Edge Functions
- [ ] Verify `analyze-document-for-tasks` has UUID validation (already deployed to dev)
- [ ] Verify `unsubscribe` has UUID validation (already deployed to dev)
- [ ] Verify `send-onboarding-email` uses `Deno.env.get('SUPABASE_URL')` instead of hardcoded URL (already deployed to dev)
- [ ] Test all edge functions reject invalid UUID formats
- [ ] Verify error messages don't leak sensitive information

#### 3.5 Existing Edge Functions (if needed)
...
- [ ] Deploy updated `TaskManager.js` with AI integration
...
#### 4.5 Security Utilities & Input Sanitization
- [ ] Deploy `utils/validation/inputSanitizer.js` utility file
- [ ] Verify input sanitization is loaded and available globally
- [ ] Deploy updated `utils/core/toast.js` with XSS fix (DOM manipulation instead of innerHTML)
- [ ] Deploy updated `utils/api/messageAPIv2.js` with message content sanitization
- [ ] Deploy updated `utils/api/taskAPI.js` with task content sanitization (titles & descriptions)
- [ ] Deploy updated `components/Events/CreateEventForm.js` with event content sanitization
- [ ] Verify all sanitization functions work correctly
...
- [ ] Test CORS policies
- [ ] **NEW:** Test XSS protection in toast messages (try submitting `<script>alert('XSS')</script>`)
- [ ] **NEW:** Test message content sanitization (verify scripts are removed from messages)
- [ ] **NEW:** Test task content sanitization (verify dangerous HTML is escaped)
- [ ] **NEW:** Test event content sanitization (verify input is properly sanitized)
- [ ] **NEW:** Test UUID validation in edge functions (submit invalid UUIDs, should reject)
- [ ] **NEW:** Test filename sanitization (verify path traversal attempts are blocked)
- [ ] **NEW:** Verify input length limits are enforced (messages 50k, task titles 200, descriptions 10k)
- [ ] **NEW:** Test that development-only migrations are NOT in production
...
5. **RLS policies** → Required for security
...
- **Note:** Includes unsubscribe fields migration (20251102000001)
