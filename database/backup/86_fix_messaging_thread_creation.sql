-- Fix messaging thread creation and lookup consistency

-- Ensure vendor_profile_id column in message_threads is TEXT type for consistency
DO $$
BEGIN
    -- Check if vendor_profile_id column exists and its type
    IF EXISTS (
        SELECT 1 
        ...
        -- Convert existing UUID data to TEXT
        ALTER TABLE message_threads ALTER COLUMN vendor_profile_id TYPE TEXT USING vendor_profile_id::TEXT;
        RAISE NOTICE 'Converted vendor_profile_id column from UUID to TEXT in message_threads';
    END IF;
END $$;

-- Update the RPC function to handle type consistency
