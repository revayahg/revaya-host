-- Working SECURITY DEFINER function to accept invitation and create role
-- This bypasses RLS and uses the invitation_token as the secret
-- Properly handles UUID casting from TEXT
-- Date: 2025-11-24
...
-- Create function that accepts invitation token and current user ID
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE au.id = current_user_id
      AND lower(trim(au.email)) = lower(trim(invitation_record.email))
  ) THEN
